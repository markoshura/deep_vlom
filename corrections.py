...
def delta_E(T,rho):
    return 2*theta(T)**2/(3*math.pi**2)*r_0(rho)**3*E_integrals() + 0.2690017*z**(5/3)

def delta_S(T,rho):
    return 2*theta(T)/(3*math.pi**2)*r_0(rho)**2*S_integrals() + 2**(1/2)*z*Diff_HI[0]/(6*math.pi*theta(T)**(1/2))
